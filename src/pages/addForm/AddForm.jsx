...
function AddForm(props) {
  ...
  const sendForm = async (isLoading = false, data = {}) => {
    ...
    try {
      ...
      const upload = await POST(api.BASE_URL + api.ENDPOINT.upload, formData, headerUpload);
      ...
      const createTokenInfo = await POST(
        api.BASE_URL + api.ENDPOINT.tokenInfo,
        {
          data: {
            ...
            chain: { disconnect: [], connect: [{ id: tokenChain }] },
            publishedAt: null,
          },
        },
        headerPost
          ...
          <div className="col-12">
            ...
            >
              <TabContext value={value}>
                <Box>
                  <TabList aria-label="lab API tabs example" className="tabParent">
                    <Tab label="1. Token Info" value="1" className="tabNum" disabled />
                    <Tab label="2. Presale Info" value="2" className="tabNum" disabled />
                    <Tab label="3. Social Media" value="3" className="tabNum" disabled />
                  </TabList>
                </Box>
                <TabPanel value="1" className="p-0">
                  <Page1 updateInput={updateInput} data={newLaunchpadData} chainsList={chainsList} />
                </TabPanel>
